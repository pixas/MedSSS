...
@register_tree("GMCTS")
class GMCTS(MCTS):
    # MCTS rollout with ground-truth information
    # def __init__(*args, **args)
    
    def future_rollout(self, node: MedMCTSNode, simulation: int, **sampling_params):
        # action = Reason()
        # lookahead two steps
        cur_value, _ = self.value_func_simulation(node)
        if self.training:
            _, simu_value = self.direct_infer_simulation(node, simulation)
        # return true_value, None
        else:
            simu_value = None
        if simu_value is not None:
            cur_value = 0.5 * (cur_value + simu_value) if abs(simu_value -1) < 1e-6 else simu_value
        return cur_value, simu_value
            # max_node.eval_node()
        # pass
    ...
    def rollout(self, node: MedMCTSNode, simu_func, simulation=20, **sampling_params):
        ...
        if self.value_function is None:
            ...
            return self.future_rollout(node, simulation, **sampling_params)
    ...
    def run_one_iter(self, **sampling_params):
        ...
        if node.visits == 0 and node.depth == 0:
            ...
            if self.value_function is not None:
                # inference mode
                value, simulation_score = self.rollout(node, self.value_func_simulation, simulation=self.config.simulation, **sampling_params)
            else:
                # pdb.set_trace()
